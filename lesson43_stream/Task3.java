...
public class Task3 {
    public static void main(String[] args) {
        ...
        autoList.add(new Auto("BMVX3",2008,6500,"BLACK",3200));
        autoList.add(new Auto("Hundai Accent",2012,5200,"Red",1400));
        autoList.add(new Auto("Kea Sportage",2016,25000,"White",2200));
        autoList.add(new Auto("Hundai Sanafe",2018,20000,"Red",2200));

        autoList.stream().forEach(x -> System.out.print(x + " "));
        System.out.println();
        autoList.stream().filter(x ->x.getColor().equals("White")).forEach( x-> System.out.println(x + " "));

        System.out.println();
        autoList.stream().filter(x ->x.getVolume() == 2200).forEach(x -> System.out.println(x + " "));

        System.out.println();
        autoList.stream().filter(x ->x.getPrice() > 10000).forEach(x -> System.out.println(x + " "));

        System.out.println();
        autoList.stream().filter(x ->x.getYearOfProd() > 2009 && x.getYearOfProd() < 2017).forEach(x -> System.out.println(x + " "));
